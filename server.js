...
app.get('/hls/:streamName/index.m3u8', async (req, res) => {
  try {
    ...
    if (response.ok) {
      ...
      console.log('Playlist content preview:', playlist.substring(0, 200));
      ...
      res.status(404).send(`Stream ${streamName} not available`);
    }
  } catch (error) {
...
app.get('/hls/:streamName/*.ts', async (req, res) => {
  try {
    const { streamName } = req.params;
    const segmentPath = req.path.replace(`/hls/${streamName}/`, '');
    const targetUrl = `http://localhost:8888/${streamName}/${segmentPath}`;
    ...
    if (response.ok) {
      ...
      console.log(`✅ HLS Segment success: ${segmentPath} (${segment.length} bytes)`);
...
app.get('/hls/:streamName/*.m3u8', async (req, res) => {
  try {
    const { streamName } = req.params;
    const playlistPath = req.path.replace(`/hls/${streamName}/`, '');
    const targetUrl = `http://localhost:8888/${streamName}/${playlistPath}`;
    ...
    if (response.ok) {
      const playlist = await response.text();
      console.log(`✅ HLS Sub-playlist success: ${playlistPath}`);
    ...
    res.status(503).send('Playlist unavailable');
  }
});
...
app.get('/api/stream/:streamName/status', async (req, res) => {
  try {
    ...
    const streamExists = pathsData.items && pathsData.items.some(item => 
      item.name === streamName && item.ready
    );
    ...
    res.status(500).json({ error: 'Stream status check failed' });
  }
});
